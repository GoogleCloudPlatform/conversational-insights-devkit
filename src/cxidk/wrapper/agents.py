...
platforms, including PolySynth, AgentStudio, and Dialogflow.
"""
...
_EXPECTED_RESOURCE_TYPES = [
    ...
    "type.googleapis.com/google.cloud.agentstudio.v1.App",
    "type.googleapis.com/google.cloud.agentstudio.v1.Agent",
]
...
class AgentStudio:
    """A client for interacting with the Agent Studio conversational agent API.

    This class provides a wrapper for the Agent Studio API, simplifying session
    creation, message sending, authentication, and the handling of long-running
    operations using the latest Agent Studio REST API endpoints.
    ...
    def __init__(
        ...
        """Initializes the AgentStudio client.
    ...
    def base_url(self) -> str:
        ...
        Returns:
            The base URL for the API endpoint.
        """
        if hasattr(self.config, "set_agent_studio_endpoint"):
            endpoint = self.config.set_agent_studio_endpoint()
        else:
            endpoint = self.config.set_polysynth_endpoint()
